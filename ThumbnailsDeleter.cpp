...
{
    startWorkTimer.setInterval (30*1000); //Start work after 30s。
    startWorkTimer.setSingleShot (true); //单发。

    ThumbnailsDirName=QDir::homePath ()+"/.cache/thumbnails/"; //缩略图目录名。
//    ThumbnailsDirName="/Ada/SoftwareDevelop/thumbnailsdeleter/Material/Thumbnails/thum/"; //缩略图目录名。
//    ThumbnailsDirName="/root/SoftwareDevelop/thumbnailsdeleter/Material/Thumbnails/thum/"; //缩略图目录名。
} //void ThumbnailsDeleter::initializeMembers()

/*!
 * \brief FakeImageEliminator::listDirFiles 开始列出各个子目录下的文件。
 */
void ThumbnailsDeleter::listDirFiles()
{
 ...
 */
void ThumbnailsDeleter::startRecognizeFile()
{
 ...
 */
void ThumbnailsDeleter::Delete1File()
{
...
{
    connect (&startWorkTimer,&QTimer::timeout,this,&ThumbnailsDeleter::startWork); //定时器超时，则开始工作。
    connect(&subDirsFutureWatcher,&QFutureWatcher<QStringList>::finished,this,&ThumbnailsDeleter::scanDirsNextLevel); //扫描下一个层次的子目录。
    connect(&fileListFutureWatcher,&QFutureWatcher<QStringList>::finished,this,&ThumbnailsDeleter::startRecognizeFile); //开始识别一个个的文件。
    connect (rangeSortSearch.getSignalProxy (),&PaSignalProxy::finished,this,&ThumbnailsDeleter::startDelete); //搜索完毕，则开始删除。
 ...
 */
void ThumbnailsDeleter::scanDirsNextLevel()
{
...
ThumbnailsDeleter::ThumbnailsDeleter(QWidget *parent) : QMainWindow(parent), ui(new Ui::ThumbnailsDeleter)
{
 ...
 */
void ThumbnailsDeleter::scanSubDir(QList<QString> scanningDirList2Scan)
{
