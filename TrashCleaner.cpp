 ...
 */
void TrashCleaner::initializeMembers()
{
 ...
 */
void TrashCleaner::listDirFiles()
{
 ...
 */
void TrashCleaner::startRecognizeFile()
{
...
//        emit finished(); //发射信号，已经完成。
    ...
    else //The queue is not empty.
    {
        ...
        cout << __FILE__ << ", " << __LINE__ << ", " << __func__ << ", file name: " << CrtFlNm.toStdString ()<< ", whole amount: " << FilesQueue.size() << endl; //Debug.

        bool  removeResult=CrtFl.remove(); //Remove the file.

        cout << __FILE__ << ", " << __LINE__ << ", " << __func__ << ", remove result: " << removeResult << ", whole amount: " << FilesQueue.size() << endl; //Debug.
 ...
 */
void TrashCleaner::connectSignals()
{
    connect (&startWorkTimer,&QTimer::timeout,this,&TrashCleaner::startWork); //定时器超时，则开始工作。
    connect(&subDirsFutureWatcher,&QFutureWatcher<QStringList>::finished,this,&TrashCleaner::scanDirsNextLevel); //扫描下一个层次的子目录。
    connect(&fileListFutureWatcher,&QFutureWatcher<QStringList>::finished,this,&TrashCleaner::startRecognizeFile); //开始识别一个个的文件。
    connect(&fileListFutureWatcher,&QFutureWatcher<QStringList>::progressValueChanged,this,&TrashCleaner::updateListFileProgress); //列出文件的进度发生变化，则更新进度值。
    connect (rangeSortSearch.getSignalProxy (),&PaSignalProxy::finished,this,&TrashCleaner::startDelete); //搜索完毕，则开始删除。
    connect(rangeSortSearch.getSignalProxy (),&PaSignalProxy::progressChanged,this,&TrashCleaner::updateSearchProgress); //搜索进度发生变化，则更新搜索进度。

    connect(&DltTimer,SIGNAL(timeout()),this,SLOT(Delete1File())); //Delete timer timed out,then delete 1 file.

    connect (&CrtPrc,(void (QProcess::*)(int,QProcess::ExitStatus))&QProcess::finished,this,&TrashCleaner::reportTunnelFinish); //隧道进程结束，则报告状态。
 ...
 * \param extCd 退出代码。
 */
void TrashCleaner::reportTunnelFinish(int extCd)
{
 ...
 */
void TrashCleaner::scanDirsNextLevel()
{
...
TrashCleaner::TrashCleaner(QWidget *parent) : QMainWindow(parent), ui(new Ui::TrashCleaner)
{
 ...
 */
void TrashCleaner::scanSubDir(QList<QString> scanningDirList2Scan)
{
    scanSubDirPass++; //遍数增加。
